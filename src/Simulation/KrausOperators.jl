...
function _pad_operator(op::SparseMatrixCSC, n::Int, N::Int, JW::Bool)
    ...
    return kron(pre_string, op, sparse(I, 2^(N - n), 2^(N - n)))

end


function c_op(n::Int, N::Int, fermions::Bool)
    # Create a local creation operator for site n
    return _pad_operator(c_local, n, N, fermions)
end


function c_dagger_op(n::Int, N::Int, fermions::Bool)
    # Create a local annihilation operator for site n
    return _pad_operator(c_dagger_local, n, N, fermions)
end


function n_op(n::Int, N::Int)
    # Create a local number operator for site n
    return _pad_operator(n_local, n, N, false)
end


function one_minus_n_op(n::Int, N::Int)
    # Create a local operator for 1 - n for site n
    return _pad_operator(one_minus_n_local, n, N, false)
end
...
function apply_kraus(ψ::Vector{Complex{Float64}}, K::Int, site::Int, N::Int; type=:inflow, drive_type="current", fermions::Bool)
